...
def load_datasets_and_vocabs(args):
    ...
    logger.log('****** After unrolling ******')
    logger.log('Train set size: {}'.format(len(train_all_unrolled)))
    logger.log('Test set size: {},'.format(len(test_all_unrolled)))
...
def get_dataset(dataset_name):
    ...
    logger.log('# Read {} Train set: {}'.format(dataset_name, len(train)))

    test = list(read_sentence_depparsed(ds_test[dataset_name]))
    logger.log("# Read {} Test set: {}".format(dataset_name, len(test)))
    return train, test
...
def get_rolled_and_unrolled_data(input_data, args):
    ...
    logger.log('*** Start processing data(unrolling and reshaping) ***')
    tree_samples = []
    ...
    logger.log('Total sentiment counter: {}'.format(total_counter))
    logger.log('Multi-Aspect-Multi-Sentiment counter: {}'.format(mixed_counter))
...
def load_and_cache_vocabs(data, args):
    ...
    if os.path.exists(cached_dep_tag_vocab_file):
        logger.log('Loading vocab of dependency tags from {}'.format(cached_dep_tag_vocab_file))
        with open(cached_dep_tag_vocab_file, 'rb') as f:
            dep_tag_vocab = pickle.load(f)
    else:
        logger.log('Creating vocab of dependency tags.')
        dep_tag_vocab = build_dep_tag_vocab(data, min_freq=0)
        logger.log('Saving dependency tags  vocab, size: {}, to file {}'.format(dep_tag_vocab['len'], cached_dep_tag_vocab_file))
    ...
    if os.path.exists(cached_pos_tag_vocab_file):
        logger.log('Loading vocab of dependency tags from {}'.format(cached_pos_tag_vocab_file))
        with open(cached_pos_tag_vocab_file, 'rb') as f:
            pos_tag_vocab = pickle.load(f)
    else:
        logger.log('Creating vocab of dependency tags.')
        pos_tag_vocab = build_pos_tag_vocab(data, min_freq=0)
        logger.log('Saving dependency tags  vocab, size: {}, to file {}'.format(pos_tag_vocab['len'], cached_pos_tag_vocab_file))
...
class ASBA_Depparsed_Dataset(Dataset):
    ...
    def __init__(self, data, args, word_vocab, dep_tag_vocab, pos_tag_vocab):
        ...
        self.tokenizer = BertTokenizer.from_pretrained(self.args.bert_model_dir)
    ...
    def convert_features_bert(self, i):
        ...
        for word in self.data[i]['aspect']:
            word_aspect_tokens = self.tokenizer.tokenize(word)
        ...
        input_aspect_ids = self.tokenizer.convert_tokens_to_ids(
            aspect_tokens)
