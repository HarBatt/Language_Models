...
def load_datasets_and_vocabs(args):
    ...
    print('Train set size: %s', len(train_all_unrolled))
    print('Test set size: %s,', len(test_all_unrolled))
...
def get_dataset(dataset_name):
    ...
    print('# Read %s Train set: %d', dataset_name, len(train))

    test = list(read_sentence_depparsed(ds_test[dataset_name]))
    print("# Read %s Test set: %d", dataset_name, len(test))
    return train, test
...
def get_rolled_and_unrolled_data(input_data, args):
    ...
    for e in input_data:
        ...
        for i in range(len(e['aspect_sentiment'])):
            ...
            # Because of tokenizer differences, aspect positions are off, so we find the index and try again.
            if len(dep_tag) == 0:
                zero_dep_counter += 1
    ...
    print('Total sentiment counter: %s', total_counter)
    print('Multi-Aspect-Multi-Sentiment counter: %s', mixed_counter)
